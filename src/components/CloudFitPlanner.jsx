...
const plannerStages = [
  { value: 'inputs', label: '1. Inputs', description: 'Shape the workload before anything else.' },
  { value: 'fit', label: '2. Recommendation', description: 'See why the top stack wins.' },
  { value: 'bill', label: '3. Services bill', description: 'Inspect what would actually exist.' },
  { value: 'preview', label: '4. IaC preview', description: 'Read the generated manifest or OpenTofu output.' },
  { value: 'handoff', label: '5. Handoff', description: 'Choose the delivery path that matches the recommendation.' },
]

function CloudFitPlanner() {
  const [scenario, setScenario] = useState(defaultCloudFitScenario)
  const [activeStage, setActiveStage] = useState('inputs')
  ...
  const activeStageMeta = plannerStages.find((stage) => stage.value === activeStage) || plannerStages[0]
        ...
        <div className="terminal-content">
            ...
            <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] xl:items-start">
              ...
              <div className="space-y-4">
                <div className="metric-card p-6 sm:p-7">
                  <p className="text-xs font-semibold uppercase tracking-[0.28em] text-primary-200">Live recommendation</p>
                ...
                <div className="rounded-[1.6rem] border border-white/10 bg-white/[0.03] p-5 sm:p-6">
                  <p className="text-xs font-semibold uppercase tracking-[0.24em] text-primary-200">Why this stays useful</p>
                  <div className="mt-4 space-y-3 text-sm leading-7 text-gray-300">
                    <p>The recommendation updates as you change the workload instead of forcing you through a dead linear wizard.</p>
                    <p>Once the scenario feels right, move into the recommendation and services-bill stages to judge whether the shortlist actually makes operational sense.</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] xl:items-start">
              ...
              <div className="space-y-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.26em] text-primary-200">Compare the shortlist</p>
          ...
          {activeStage === 'bill' ? (
            <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.06fr)_minmax(0,0.94fr)] xl:items-start">
              <div className="rounded-[1.6rem] border border-white/10 bg-white/[0.03] p-5 sm:p-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.26em] text-primary-200">3. Services listing</p>
          ...
          {activeStage === 'preview' ? (
            <div className="rounded-[1.6rem] border border-white/10 bg-white/[0.03] p-5 sm:p-6">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.26em] text-primary-200">4. IaC preview</p>
                ...
                <div className="flex flex-wrap gap-2">
                  {previewModes.map((mode) => (
                    <button
                      ...
                      className={`segmented-button ${previewMode === mode.value ? 'segmented-button-active' : ''}`}
                    >
                      {mode.label}
                    </button>
                  ))}
          ...
          {activeStage === 'handoff' ? (
            <div className="space-y-4">
              <div className="rounded-[1.6rem] border border-white/10 bg-white/[0.03] p-5 sm:p-6">
                <p className="text-xs font-semibold uppercase tracking-[0.26em] text-primary-200">5. Delivery handoff</p>
                <h4 className="mt-3 text-2xl font-semibold text-white">Choose the next step that fits the stack decision.</h4>
                <p className="mt-3 max-w-3xl text-sm leading-7 text-gray-400">
                  The recommendation is currently pointing toward{' '}
                  <span className="font-semibold text-white">
                    {discussOfferPresets[recommended.recommendedOffer].optionLabel}
                  </span>
                  . Use the review when you want a sharper architecture decision, or move straight into the deploy pack when the bill of materials already looks directionally right.
                </p>
              </div>
